...
class MikrobiologiFormController extends Controller
{
    ...
    public function update(Request $request, MikrobiologiForm $mikrobiologi_form)
    {
        ...
        // Build payload without tgl_pengamatan initially
        $payload = [
            'title' => $validated['title'],
            'no' => $validated['no'],
            'judul_tabel' => $validated['judul_tabel'] ?? null,
        ...
        if ($request->has('observations')) {
            ...
            foreach ($request->input('observations') as $obs) {
                if (!empty($obs['id'])) {
                    ...
                    if ($model) {
                        $updateData = ['keterangan' => $obs['keterangan'] ?? null];
                        if (!empty($obs['tanggal'])) {
                            $updateData['tanggal'] = $obs['tanggal'];
                        }
            ...
            if ($first) {
                $mikrobiologi_form->tgl_pengamatan = $first->tanggal;
            } else {
                $mikrobiologi_form->tgl_pengamatan = $validated['tgl_pengamatan'] ?? $validated['tgl_inokulasi'];
            }
