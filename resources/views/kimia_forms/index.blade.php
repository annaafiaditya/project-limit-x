...
<div class="max-w-7xl mx-auto py-6 animate-fade-in-up">
    <div class="flex justify-between items-center mb-4 animate-fade-in-up">
        ...
        <div class="flex gap-2 align-items-center">
            ...
            <form method="GET" action="{{ route('kimia.create') }}" class="d-flex align-items-center gap-2 bg-white border border-primary rounded px-2 py-1 shadow-sm" style="max-width: 340px;">
    ...
    <div class="mb-3 d-flex align-items-center justify-content-between animate-fade-in-up">
        <div class="d-flex align-items-center gap-2">
            <form method="GET" action="{{ route('kimia.index') }}" class="d-inline">
            ...
            @if(request('approval') == 'pending')
                <a href="{{ route('kimia.index') }}" class="btn btn-outline-secondary">Tampilkan Semua</a>
            @endif
        </div>
        <div>
    ...
    <form method="GET" action="" class="row g-2 align-items-end mb-4 animate-fade-in-up">
        ...
        <div class="col-md-1">
            <a href="{{ route('kimia.index') }}" class="btn btn-outline-secondary w-100">Reset</a>
        </div>
    </form>
    ...
    <div class="bg-white shadow rounded-lg overflow-x-auto animate-fade-in-up" style="animation-delay:.15s; animation-duration:.8s;">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody>
                ...
                <tr class="hover:bg-yellow-50 cursor-pointer" onclick="window.location='{{ route('kimia.show', $form) }}'" data-entry-count="{{ $form->entries->count() }}" data-approval-count="{{ $form->signatures->where('status', 'accept')->count() }}">
