...
<body>
    ...
    <div class="form-info">
        ...
        <div>
            <strong>Dibuat:</strong> {{ \Carbon\Carbon::parse($kimia_form->created_at)->format('d/m/Y H:i') }}
        </div>
    </div>
    ...
    @php 
        ...
        $lightColors = ['#dbeafe', '#e9d5ff', '#cffafe', '#d1fae5', '#fef3c7', '#fee2e2', '#f7fee7', '#fed7aa'];
        $color = $colors[$loop->index % count($colors)];
        $lightColor = $lightColors[$loop->index % count($lightColors)];
    @endphp
    <div class="table-section">
        <div class="table-title" style="color: {{ $color }}; background: {{ $lightColor }}; border-left: 4px solid {{ $color }};">{{ $table->name }}</div>
                ...
                <thead>
                    <tr>
                        @foreach($table->columns as $col)
                            <th style="background: {{ $color }}; border: 1px solid {{ $color }};">{{ $col->nama_kolom }}</th>
                        @endforeach
                    </tr>
                </thead>
                <tbody>
                    @foreach($table->entries as $entry)
                        <tr style="{{ $loop->even ? 'background: ' . $lightColor . ';' : '' }}">
                            @foreach($table->columns as $col)
                                <td>
